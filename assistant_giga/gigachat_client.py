...
# Отключаем предупреждение о непроверенном HTTPS (сертификаты Сбера)
...
class GigaChatClient:
    ...
    # Проверка SSL-сертификата (False — для работы с сертификатами Сбера)
    VERIFY_SSL = False  # Сбер использует сертификат, не проходящий стандартную проверку
    ...
    def __init__(self, auth_key: str = None, rq_uid: str = None):
        ...
        self._embeddings_fallback_warned = False  # один раз предупредить о fallback
    ...
    def get_embeddings(self, texts: List[str], model: str = "Embeddings") -> List[List[float]]:
        ...
        try:
            ...
            if not self._embeddings_fallback_warned:
                self._embeddings_fallback_warned = True
                print(f"⚠️  Embeddings API недоступен (402 = платный тариф), используется fallback: {e}")
                print("   Семантический поиск будет неточным. Для качества RAG подключите Embeddings в личном кабинете GigaChat или используйте локальные эмбеддинги.")
